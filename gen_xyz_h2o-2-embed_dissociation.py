...
def create_xyz_files(output_dir, distance_bohr):
    ...
    with open(os.path.join(output_dir, "tot.xyz"), "w") as f:
        ...
        for atom in water1:
            f.write(f"{'O' if np.all(atom == 0) else 'H'} {atom[0]:.6f} {atom[1]:.6f} {atom[2]:.6f}\n")
        for atom in water2:
            f.write(f"{'O' if atom[1] == 0 and atom[2] == 0 else 'H'} {atom[0]:.6f} {atom[1]:.6f} {atom[2]:.6f}\n")

    with open(os.path.join(output_dir, "frag1.xyz"), "w") as f:
        f.write("3\n")
        f.write(f"Water dimer fragment 1 at d = {distance:.2f} A = {distance_bohr:.1f} Bboohhrr, method=dft\n")
        for atom in water1:
            f.write(f"{'O' if np.all(atom == 0) else 'H'} {atom[0]:.6f} {atom[1]:.6f} {atom[2]:.6f}\n")

    with open(os.path.join(output_dir, "frag2.xyz"), "w") as f:
        f.write("3\n")
        f.write(f"Water dimer fragment 2 at d = {distance:.2f} A = {distance_bohr:.1f} Bboohhrr, method=wf\n")
        for atom in water2:
            f.write(f"{'O' if atom[1] == 0 and atom[2] == 0 else 'H'} {atom[0]:.6f} {atom[1]:.6f} {atom[2]:.6f}\n")
